...
class ModelHandler(BaseHandler):
    ...
    def initialize(self, context):
        ...
        state_dict = torch.load('003_realSR_BSRGAN_DFO_s64w8_SwinIR-M_x4_GAN.pth')
        param_key_g = "params_ema"
        self.swinir_model.load_state_dict(state_dict[param_key_g] if param_key_g in state_dict.keys() else state_dict, strict=True)
        self.swinir_model.to(self.device)
    ...
    def inference(self, img_lr):
        ...
        # Run inference call to engine here and return output image
        with torch.no_grad():
            _, _, h_prev, w_prev = img_lr.size()
            
            h_pad = (h_prev // 8 + 1) * 8 - h_prev
            w_pad = (w_prev // 8 + 1) * 8 - w_prev
            img_lr = torch.cat([img_lr, torch.flip(img_lr, [2])], 2)[:, :, :h_prev + h_pad, :]
            img_lr = torch.cat([img_lr, torch.flip(img_lr, [3])], 3)[:, :, :, :w_prev + w_pad]

            output = self.swinir_model(img_lr)
            output = output.to(self.device)
            output = output[..., :h_prev, :w_prev]
